    ...
    try {
        ...
        if (analysis.modelPageUrl) {
            try {
                ...
                console.error(`${tag} getModelMarketData failed: ${e.message}`);
            }
        }
        else {
...
async function main() {
    ...
    for (let b = 0; b < batches.length; b++) {
        ...
        const batchResults = await Promise.all(batch.map((card, i) => processListing(card, batchStart + i, listings.length)));
    ...
    const sorted = results.sort((a, b) => VERDICT_ORDER[a.score.verdict] - VERDICT_ORDER[b.score.verdict]);
    const counts = {

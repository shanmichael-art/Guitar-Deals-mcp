...
export async function getModelMarketData(args) {
    ...
    if (targetCondition) {
        await page.evaluate((cond) => {
            ...
            const tab = tabs.find(el => el.textContent?.trim().toLowerCase() === cond.toLowerCase());
            tab?.click();
        }, targetCondition);
    ...
    const data = await page.evaluate(() => {
        ...
        const rangeText = document.querySelector(".csp-price-estimates__values, .csp-price-estimates")
            ?.textContent?.trim() ?? "";
        ...
        const estimatedValue = lowPrice !== null && highPrice !== null
        ...
        const txRows = Array.from(document.querySelectorAll(".csp-transaction-table-container tr, [class*='transaction'] tr"));
        ...
        for (const card of Array.from(document.querySelectorAll(".rc-listing-card"))) {
            ...
            if (!url || seen.has(url))
                continue;
            seen.add(url);
            const cardTitle = card.querySelector(".rc-listing-card__title")?.textContent?.trim() ?? "";
            const priceText = card.querySelector(".rc-price-block__price")?.textContent?.trim() ?? "";
            const price = priceText ? Number(priceText.replace(/[$,]/g, "")) : 0;
            if (!price)
                continue;
            lowestListings.push({ title: cardTitle, url, price, shipping: 0, total: price });
            if (lowestListings.length >= 5)
                break;
        }

...
import type { ListingAnalysis } from "../types.js";
...
export async function analyzeListing(
  args: z.infer<typeof AnalyzeListingSchema>
  ...
  const raw = await page.evaluate(() => {
    ...
    const priceText =
      document.querySelector(".price-display")?.textContent?.trim() ?? "";
    ...
    const condText =
      document.querySelector(".condition-display__label")?.textContent?.trim() ?? "";
    const condMatch =
    ...
    const descEl =
      document.querySelector(
        "[class*='description'], [class*='seller-notes'], [class*='item2-description']"
      ) ??
    ...
    const modelLink =
      (Array.from(document.querySelectorAll('a[href*="/p/"]')) as HTMLAnchorElement[])
  ...
  if (raw.needsZipCalc) {
    try {
      ...
      const updateClicked = await page.evaluate(() => {
        ...
        const byId = document.querySelector("#rc-modal-action-primary") as HTMLElement | null;
        if (byId) { byId.click(); return true; }
        const allBtns = Array.from(document.querySelectorAll("button, input[type='submit']")) as HTMLElement[];
        const updateBtn = allBtns.find(b => /^update$/i.test(b.textContent?.trim() ?? "") || /^update$/i.test((b as HTMLInputElement).value ?? ""));
      ...
      const updatedShipText = await page.evaluate(() =>
        document.querySelector(".listing-shipping-display")?.textContent?.trim() ?? ""
      );

...
server.tool(
  "scan_deals_marketplace",
  "Scan a Reverb marketplace page and return raw candidate listings.",
  {
    marketplaceUrl: z.string().url(),
    maxListings: z.number().int().min(1).max(100).default(25)
  },
  async (args) => {
    const result = await scanDealsMarketplace(
      ScanDealsMarketplaceSchema.parse(args)
    );
...
server.tool(
  "analyze_listing",
  "Open a Reverb listing and extract details, model page URL, and issues.",
  {
    listingUrl: z.string().url()
  },
  async (args) => {
    const result = await analyzeListing(
      AnalyzeListingSchema.parse(args)
    );
...
server.tool(
  "get_model_market_data",
  "Open a Reverb model page and extract canonical title, price guide context, and lowest active listings.",
  {
    modelPageUrl: z.string().url(),
    condition: z.string().nullable().optional()
  },
  async (args) => {
    const result = await getModelMarketData(
      GetModelMarketDataSchema.parse(args)
    );
...
server.tool(
  "score_deal",
  "Calculate strong offer and verdict for a guitar listing.",
  {
    askPrice: z.number(),
